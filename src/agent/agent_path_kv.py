...
class AgentPath_KV(nn.Module):
    def __init__(self, config, model):
        ...
        self.use_cache = True  # only works during inference
    ...
    def reset(self, batch):
        ...
        if not self.training and self.use_cache:
            kv_cache = self.model.init_kv_cache(self.text_features, self.text_padding_mask)
            self.text_kv_cache = [(k, v) for _, _, k, v in kv_cache]
            self_attention_kv_cache = [(k, v) for k, v, _, _ in kv_cache]
            self.graph = BatchGraph_trajectory(batch, self_attention_kv_cache)
        else:
            self.graph = BatchGraph_trajectory(batch, None)
    ...
    def _take_action(self, obs):
        ...
        if not self.training and self.use_cache:
            kv_cache, path_padding_mask, local_features, local_padding_mask = self.graph.get_path_features(obs)
            new_kv_cache = []
            head_num = self.model.nhead
            head_dim = int(self.model.hidden_dim / head_num)
            for i, (k, v) in enumerate(kv_cache):
                B = k.shape[0]
                k = k.reshape(B, -1, head_num, head_dim).permute(0, 2, 1, 3)
                v = v.reshape(B, -1, head_num, head_dim).permute(0, 2, 1, 3)
                new_kv_cache.append((k, v) + self.text_kv_cache[i])
            ...
            next_kv_cache, local_tokens = self.model.forward_MAM(
            ...
            self.graph.update_path_feature(obs, next_kv_cache, local_tokens)
        else:
            ...
            self.graph.update_path_feature(obs, None, local_tokens)
...
class BatchGraph_trajectory:
    """
    Here, to support the kv cache, I use a simpler method to maintain trajectory
    instead of previous stack strategy.
    """
    def __init__(self, batch, self_attention_kv_cache):
        ...
        self.use_cache = self_attention_kv_cache is not None
        for i, graph in enumerate(self.graph_batch):
            if self.use_cache:
                kv_cache = [(k[i, :, :], v[i, :, :]) for k, v in self_attention_kv_cache]  # j+1 to remove the stop embedding
            else:
                kv_cache = None
            graph.add_node(self.start_point[i], kv_cache=kv_cache, trajectory=[self.start_point[i]])
    ...
    def update_graph(self, obs, candidate_features,):
        ...
        for i, candidates in enumerate(obs['candidates']):
            ...
            trajectory = self.graph_batch[i].nodes[v1]['trajectory']
            for j in range(candidates.shape[0]):
                ...
                self.graph_batch[i].add_node(v2, trajectory=trajectory + [v2])
    ...
    def get_path_features(self, obs):
        ...
        if not self.use_cache:
            ...
            for i, graph in enumerate(self.graph_batch):
                ...
                trajectory = graph.nodes[current_viewpoint_id]['trajectory']
            ...
            for i, graph in enumerate(self.graph_batch):
                current_viewpoint_id = obs.at[i, 'viewpoint_id']
                kv_cache.append(graph.nodes[current_viewpoint_id]['kv_cache'])
                ...
                local_features.append(local_feature)
            # prepare kv cache, list_B[list_layer[tuple(N, C)]] to list_layer[tuple(B, N, C)]
            kv_cache = list(zip(*kv_cache))  # list_layer[list_B(tuple(N, C))]
            lengths = [k.shape[0] for k, _ in kv_cache[0]]
            for i, layer_cache in enumerate(kv_cache):
                self_attention_key = pad_sequence([k for k, _ in layer_cache], batch_first=True)  # (B, N, C)
                self_attention_value = pad_sequence([v for _, v in layer_cache], batch_first=True)  # (B, N, C)
                kv_cache[i] = (self_attention_key, self_attention_value)
            path_padding_mask = utils.length_to_mask(lengths, device=device)

            local_lengths = [f.shape[0] for f in local_features]
            local_padding_mask = utils.length_to_mask(local_lengths, device=device)
            local_features = pad_sequence(local_features, batch_first=True)
            return kv_cache, path_padding_mask, local_features, local_padding_mask

    def update_path_feature(self, obs, self_attention_kv_cache, path_feature):
        """
        self_attention_kv_cache: list_layer[tuple((B, M, C), (B, M, C))]
        """
        for i, graph in enumerate(self.graph_batch):
            v1 = obs.at[i, 'viewpoint_id']
            ...
            if self.use_cache:
                previous_kv_cache = graph.nodes[v1]['kv_cache']
            for j, v2 in enumerate(self.to_be_updated[i]):
                if graph.has_node(v2) and 'kv_cache' in  graph.nodes[v2]:
                    continue

                kv_cache = None
                if self.use_cache:
                    kv_cache = [(
                                    torch.cat([previous_kv_cache[layer_id][0], k[i, j+1:j+2, :]], dim=0),
                                    torch.cat([previous_kv_cache[layer_id][1], v[i, j+1:j+2, :]], dim=0)
                                )
                                for layer_id, (k, v) in enumerate(self_attention_kv_cache)]  # j+1 to remove the stop embedding
                graph.add_node(v2, path_feature=path_feature[i, j+1], kv_cache=kv_cache)
